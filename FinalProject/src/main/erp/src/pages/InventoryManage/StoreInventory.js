...
const StoreInventory = () => {
    ...
    const fetchProductInventory = useCallback(async () => {
        const response = await fetch(`http://localhost:8080/api/store/inventory/products?page=${productPage}&size=${productSize}&search=${productSearch}`);
        const data = await response.json();
        setProductInventory(data);
    }, [productPage, productSize, productSearch]);

    const fetchMaterialInventory = useCallback(async () => {
        const response = await fetch(`http://localhost:8080/api/store/inventory/materials?page=${materialPage}&size=${materialSize}&search=${materialSearch}`);
        const data = await response.json();
